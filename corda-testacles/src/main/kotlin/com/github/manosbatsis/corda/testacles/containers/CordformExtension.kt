...
import com.github.manosbatsis.corbeans.test.containers.NodeConfig
    ...
    fun startContainers(context: ExtensionContext) {
        if (instances.isEmpty()) {
            ...
            fsBindDirs.nodeDirs.forEach { nodeDir ->
                startContainerFromNodedir(fsBindDirs, nodeDir)
            }
        }
    }

    private fun startContainerFromNodedir(
            fsBindDirs: FsBindDirs, nodeDir: File, network: Network? = Network.newNetwork()
    ) {
